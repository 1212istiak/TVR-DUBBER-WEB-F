import { api, setAdminPassword, clearAdminPassword } from './api.js';
...
// There's no session token — the plain password itself is what's stashed
// locally so a returning admin doesn't have to retype it every visit. It's
// re-validated against the server on every admin request anyway.
const PASSWORD_KEY = 'tvr_admin_password';
let onDataChanged = () => {};
...
export function initAdmin({ onChanged }) {
  ...
  // Silently re-check a stored password so a returning admin doesn't need
  // to log in every visit. If it's since been changed elsewhere, drop it.
  const stored = getStoredPassword();
  if (stored) {
    setAdminPassword(stored);
    api.login(stored).catch(() => {
...
function setupHiddenReveal() {
  ...
  target.addEventListener('click', () => {
    ...
    if (clicks >= ADMIN_REVEAL_CLICKS) {
      clicks = 0;
      if (getStoredPassword()) openAdminPanel();
